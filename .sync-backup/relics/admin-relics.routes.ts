...
import { getRelicEffect, MAX_VAULT_SIZE, refreshRelicOverrides } from './relics.config';
...
// POST /api/admin/relics/overrides/:key
router.post('/relics/overrides/:key', async (req: AuthRequest, res, next) => {
  try {
    const { key } = req.params;
    const { name, description, type, iconUrl } = req.body;

    const base = getRelicEffect(key);
    if (!base) {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: '遗物不存在' } });
    }
    ...
    await prisma.relicOverride.upsert({
      where: { key },
      create: { key, ...data },
      update: data,
    });

    await refreshRelicOverrides(prisma);

    res.json({ success: true, message: '图鉴已更新', data: { key, ...data } });
  } catch (error) {
    next(error);
  }
});

// GET /api/admin/relics/overrides
router.get('/relics/overrides', async (req: AuthRequest, res, next) => {
  try {
    const overrides = await prisma.relicOverride.findMany();
