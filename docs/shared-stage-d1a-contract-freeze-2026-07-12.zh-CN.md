...
数据库迁移 `20260712103000_stage_contract_v1_1` 为 `StageChannel` 增加 `participantUserIds`（JSON 字符串，默认 `[]`）。`20260712113000_stage_runtime_integrity` 则补上非空 `scopeKey`，修复 SQLite 对 nullable composite unique 不约束多个 MAIN_ROOM 的问题；现存重复主轨道被保留为 `legacy-main:<id>`，不会再被投影为当前主舞台。`20260712120000_stage_actor_binding_scope` 为绑定角色 actor 增加非空 scopeKey；同一 channel、用户与角色只保留按 `updatedAt`（再按 id）最新的一条，旧重复运行态删除。该迁移会将所有 `StageSnapshot.projectionJson` 中被删除的 actor 标识改为规范 actor，保证恢复与下一命令只使用现役 actor；不可变 `StageEvent` 仍保留原始 actorId。现存私密轨道在回填参与者前只会对具备 `canManageStage` 的 KP 可见；这是一项保守访问控制，不会把私密轨道泄露给普通成员。
