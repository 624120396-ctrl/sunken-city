...
5. 由独立审计检查签名 URL（正确子域 200/206、过期/篡改/主站 host 403 或 404、目录穿越不可达）和 Nginx 仅暴露 delivery。响应包含 `Referrer-Policy: no-referrer`；跨域仅允许 `https://coc.city` 的无凭据 GET/OPTIONS，避免 bearer query 落入 Referer。
...
`stage-assets-import.ts` 仅接受目标房间的 active 成员作为上传者；`PRIVATE_TARGETS` 会去重并要求每个目标仍是 active 成员。它以 `roomId + kind + sha256` 查找现有记录：ACL 与上传者完全相同则复用，冲突或多条记录立即拒绝。文件先写入同目录临时文件、fsync、收紧为 0640 后原子 rename；数据库创建失败会删除刚写入的文件。

`stage-wusoan-gray-seed.ts` 是不可泛化的 WUSOAN 工具，拒绝其他 `--room`。它要求 KP/PL 都是 active 成员、KP 具 KP/owner 能力、PL 是绑定角色卡的 PLAYER，并校验背景、两张立绘、BGM 的 room/kind/visibility/uploader。每名演员会创建或复用合法的 `PortraitPack`/`PortraitVariant`，`StageActorState.portraitVariantId` 只保存真正的 variant ID；snapshot 仍按冻结契约投射 `portraitAssetId`。`--apply` 与 `--enable-room-stage` 保持分离，enable 在事务内再次执行完整校验。
