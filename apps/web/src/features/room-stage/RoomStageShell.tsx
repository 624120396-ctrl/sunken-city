...
export function RoomStageShell({ roomId, socketRef, connected, isMobile }: { roomId: string; socketRef: RefObject<Socket | null>; connected: boolean; isMobile: boolean }) {
  ...
  const audio = useStageAudio(model || { channelId: 'none', channelKind: 'MAIN_ROOM', scene: { title: '' }, actors: [], capabilities: { canUseStage: false, canControlOwnStageActor: false, canManageStage: false, canManageStageAssets: false, canExportStageReplay: false }, viewer: { userId: '', kind: 'OBSERVER', roomRole: '' }, revision: 0 }, preference === 'stage' && Boolean(stage.status?.stageEnabled && stage.status.enabled && stage.status.capabilities.canUseStage) && pixiAvailable);
