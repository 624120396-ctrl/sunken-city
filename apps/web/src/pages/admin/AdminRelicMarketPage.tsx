...
export function AdminRelicMarketPage() {
  ...
  const [overrideMap, setOverrideMap] = useState<Record<string, { name?: string; description?: string; type?: string; iconUrl?: string }>>({});

  const mergedRelics = Object.entries(RELIC_REGISTRY).map(([key, base]) => {
    const ov = overrideMap[key] || {};
    return {
      ...base,
      ...(ov.name && { name: ov.name }),
      ...(ov.description && { description: ov.description }),
      ...(ov.type && { type: ov.type as any }),
      ...(ov.iconUrl && { iconUrl: ov.iconUrl }),
    };
  });

  const relicOptions = mergedRelics.map((r) => ({ key: r.key, name: r.name, rarity: r.rarity }));

  const filteredCodex = mergedRelics.filter((r) => {
  ...
  useEffect(() => {
    ...
    else if (tab === 'codex') loadOverrides();
  ...
  const loadOverrides = async () => {
    try {
      const res = await getRelicOverrides();
      if (res.success) {
        const map: Record<string, any> = {};
        (res.data.overrides || []).forEach((ov: any) => {
          map[ov.key] = {
            name: ov.name ?? undefined,
            description: ov.description ?? undefined,
            type: ov.type ?? undefined,
            iconUrl: ov.iconUrl ?? undefined,
          };
        });
        setOverrideMap(map);
      }
  ...
  const openEditModal = (key: string) => {
    ...
    setEditDescription(ov.description ?? base.description ?? '');
    setEditType(ov.type ?? base.type ?? '');
    setEditIconUrl(ov.iconUrl ?? '');
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const payload: any = {};
      const base = RELIC_REGISTRY[editKey];
      if (editName !== (base?.name || '')) payload.name = editName;
      if (editDescription !== (base?.description || '')) payload.description = editDescription;
      if (editType !== (base?.type || '')) payload.type = editType;
      if (editIconUrl) payload.iconUrl = editIconUrl;

      const res = await updateRelicOverride(editKey, payload);
      if (res.success) {
        setToast({ message: '图鉴已更新', type: 'success' });
        setOverrideMap((prev) => ({
          ...prev,
          [editKey]: {
        ...
        setToast({ message: res.error?.message || '更新失败', type: 'error' });
      }
    } catch {
                ...
                {filteredCodex.map((r) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2">
                        {r.iconUrl ? (
                          <img src={r.iconUrl} alt="" className="w-6 h-6 rounded object-cover" />
                        ) : (
                          <div className="w-6 h-6 rounded border border-coc-border flex items-center justify-center text-coc-text-muted">
                            <ImageIcon size={14} />
                          </div>
                        )}
                    ...
                    <div className="mt-3 pt-2 border-t border-coc-border flex justify-between items-center">
                      <button
                        onClick={() => openEditModal(r.key)}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs border border-coc-text-muted/40 text-coc-text-muted hover:bg-coc-mist"
                      >
                        <Edit3 size={12} />
                        编辑图鉴
                      </button>
      ...
      {editKey && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4">
          <RuneBorder variant="gold" intensity="normal" showCorners>
            <div className="bg-coc-bg-secondary p-6 w-full max-w-md">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-coc-parchment">编辑遗物图鉴</h3>
                <button onClick={closeEditModal} className="text-coc-text-muted hover:text-coc-parchment">
                  <X size={20} />
                </button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs text-coc-parchment-dim mb-1">名称</label>
                  <input
                    type="text"
                    value={editName}
                ...
                <div>
                  <label className="block text-xs text-coc-parchment-dim mb-1">文字介绍</label>
                  <textarea
                    value={editDescription}
                    onChange={(e) => setEditDescription(e.target.value)}
                    rows={4}
                    className="w-full bg-coc-bg-tertiary border border-coc-border rounded px-3 py-2 text-sm text-coc-parchment outline-none focus:border-coc-gold resize-none"
                  />
                </div>
                <div>
                  <label className="block text-xs text-coc-parchment-dim mb-1">类别</label>
                  <select
                    value={editType}
                ...
                <div>
                  <label className="block text-xs text-coc-parchment-dim mb-1">图标</label>
                  <div className="flex items-center gap-2 mb-2">
                    <input
                      ...
                      placeholder="https://..."
                      className="flex-1 bg-coc-bg-tertiary border border-coc-border rounded px-3 py-2 text-sm text-coc-parchment outline-none focus:border-coc-gold"
                    />
                    {editIconUrl ? (
                      <img
                        src={editIconUrl}
                        alt="icon"
                        className="w-8 h-8 rounded border border-coc-border object-cover"
                        onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                      />
                    ) : (
                      <div className="w-8 h-8 rounded border border-coc-border flex items-center justify-center text-coc-text-muted">
                        <ImageIcon size={16} />
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2 py-1.5 rounded text-xs border border-coc-border text-coc-parchment hover:bg-coc-mist"
                    >
                      <Upload size={12} />
                      本地上传
                    </button>
                    <button
                      type="button"
                      onClick={() => setAiPrompt((p) => (p ? '' : '克苏鲁风格遗物图标，暗黑神秘，游戏道具风格，纯色背景'))}
                      className="inline-flex items-center gap-1 px-2 py-1.5 rounded text-xs border border-coc-gold/40 text-coc-gold hover:bg-coc-gold/10"
                    >
                      <Wand2 size={12} />
                      AI 生成
                    </button>
                  ...
                  {aiPrompt !== '' && (
                    <div className="mt-2 flex items-center gap-2">
                      <input
                        ...
                        placeholder="描述你想要的图标..."
                        className="flex-1 bg-coc-bg-tertiary border border-coc-border rounded px-3 py-1.5 text-xs text-coc-parchment outline-none focus:border-coc-gold"
                      />
                      <button
                        type="button"
                        onClick={handleAiGenerateIcon}
                        disabled={aiLoading}
                        className="px-3 py-1.5 rounded bg-coc-gold text-coc-abyss text-xs font-medium hover:bg-coc-gold-glow disabled:opacity-50"
                      >
                        {aiLoading ? (
