...
export async function codingChat(
  ...
  const apiKey = CODING_PLAN_API_KEY;
  if (!apiKey) {
    throw new Error('Missing ARK_API_KEY environment variable');
  }

  const model = options.model || 'ark-code-latest';
  ...
  if (!response.ok) {
    ...
    throw new Error(`Coding Plan API error: ${response.status} ${text}`);
  }
  ...
  if (!content) {
    throw new Error('Coding Plan API returned empty content');
  }
...
/**
 * Schema 变更影响审查器
 * 把 schema.prisma 的 diff 发给 DeepSeek-V3.2 (通过 Auto 路由)，返回影响清单
 */
...
/**
 * 代码测试生成器
 * 读取一个 Express 路由文件，生成 vitest + supertest 测试用例
 */
